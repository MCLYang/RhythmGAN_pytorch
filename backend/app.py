...
def opt_global_inti():
    ...
    #training parameters
        ...
        parser.add_argument('--vis_perEpoch', type=int,default=100,help="vis_perEpoch")
        parser.add_argument('--save_fig', type=lambda x: (str(x).lower() == 'true'),default=True,help="Save the sample plot during the testing")
        parser.add_argument('--show_fig', type=lambda x: (str(x).lower() == 'false'),default=False,help="show the sample plot during the testing")
...
def save_audio(drum_generated,signature):
    # signature = {'epoch':epoch,'genre':GENRES[label_random[0,0]]}
    # plt.savefig('./img/%d_Epoch_%s.png'%(signature['epoch'],signature['genre']))
    audio_data,_ = get_audio_data(np.squeeze(drum_generated))
    current_path = os.path.join(root_style_made, './img/%d_Epoch_%s.wav'%(signature['epoch'],signature['genre'])) 
    wavfile.write(current_path, 44100, audio_data)
...
def main():
    ...
    for epoch in range(opt.epoch_ckpt,opt.epoch_max):
        ...
        if(epoch%opt.vis_perEpoch == 0):
            ...
            # save_audio(drum_generated,signature = signature)            
...
def all():
    img_files = glob("./img/*.png")
    img_urls = [url_for('static', filename=img.split('/')[-1]) for img in img_files]
